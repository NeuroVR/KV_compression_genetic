...
__global__ void calc_gqa_score_kernel(
    ...
    bool has_outliers = (outlier_counts > 0) && (key_outlier_quant != nullptr) && 
                       (key_outlier_norm != nullptr) && (outlier_indices != nullptr) &&
                       (outlier_sketch_dim > 0);
    ...
    const uint8_t* outlier_ind = nullptr;
    const uint8_t* outlier_quant = nullptr;
    const T* outlier_norm = nullptr;
    
    if (has_outliers) {
        int base_index_outlier_indices = (k_bh * n_size * outlier_counts) + (n * outlier_counts);
        outlier_ind = outlier_indices + base_index_outlier_indices;
        
        int base_index_outlier_quant = (k_bh * n_size * group_size * outlier_hash_dim) + (n * group_size * outlier_hash_dim) + (gIdx * outlier_hash_dim);
        outlier_quant = key_outlier_quant + base_index_outlier_quant;
        
        outlier_norm = key_outlier_norm + base_index_key_norm;
    }
    ...
    for (size_t chnl_tile{0}; chnl_tile < sketch_dim; chnl_tile += (8*WARP_SIZE)){
        ...
        for (size_t grp_tile{wIdx}; grp_tile < WARP_SIZE; grp_tile += WARPS_PER_BLOCK) {
            ...
            if (has_outliers && chnl_tile + 8*threadLane < outlier_sketch_dim) {
            ...
            for (size_t gqa_idx{0}; gqa_idx < gqa_group_size; gqa_idx++) {
                ...
                for (int shift = 0; shift < 8; shift++) {
                    int safe_gqa_idx = (gqa_idx < GQA_GROUP_SIZE) ? gqa_idx : (GQA_GROUP_SIZE - 1);
                    int safe_threadLane = (threadLane < WARP_SIZE) ? threadLane : (WARP_SIZE - 1);
                    int safe_shift = (shift < 8) ? shift : 7;
    
                    float q_sketch_val = shared_q_sketch[safe_gqa_idx][safe_threadLane][safe_shift] - 
                        shared_q_outliers_sketch[safe_gqa_idx][safe_threadLane][safe_shift];
    ...
    if (wIdx < gqa_group_size) {
        ...
        float norm_k = 0.0f;
        float norm_otlr = 0.0f;
        
        float k_norm_val = convert_to_float<T>(k_norm[threadLane]);
        if (has_outliers) {
            norm_otlr = convert_to_float<T>(outlier_norm[threadLane]);
            float squared_diff = fmaxf(0.0f, k_norm_val * k_norm_val - norm_otlr * norm_otlr);
            norm_k = sqrtf(squared_diff);
        } else {
            norm_k = k_norm_val;
        }
        
        float score = scl * norm_k * shared_innprod[wIdx][threadLane];
        
        if (has_outliers) {
            float scl_otlr = sqrtf(M_PI_2) / static_cast<float>(outlier_sketch_dim);
            score += scl_otlr * norm_otlr * shared_outlier_innprod[wIdx][threadLane];
        }
...
torch::Tensor QJLGQAScoreCudaTemplate(
    ...
    auto options = torch::TensorOptions().device(torch::kCUDA, key_quant.get_device()).dtype(torch::kFloat);
    ...
    calc_gqa_score_kernel<<<numBlocks, threadsPerBlockDim>>>(
        query_states_ptr,
