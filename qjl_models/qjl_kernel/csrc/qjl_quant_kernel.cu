...
torch::TensorOptions getOptionsForType(const std::type_info& typeInfo, int device_id) {
    if (typeInfo == typeid(c10::Half)) {
        return torch::TensorOptions().device(torch::kCUDA, device_id).dtype(torch::kHalf);
    } else if (typeInfo == typeid(float)) {
        return torch::TensorOptions().device(torch::kCUDA, device_id).dtype(torch::kFloat);
    } else if (typeInfo == typeid(at::BFloat16)) {
        return torch::TensorOptions().device(torch::kCUDA, device_id).dtype(torch::kBFloat16);
    } else {
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> QJLQuantCudaTemplate(
    ...
    auto options = torch::TensorOptions().device(torch::kCUDA,  key_states.get_device()).dtype(torch::kUInt8);
    auto options_outlier_norm = getOptionsForType(typeid(T), key_states.get_device());
