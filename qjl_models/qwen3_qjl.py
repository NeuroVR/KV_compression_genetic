...
class Qwen3Attention_QJL(nn.Module):
    def __init__(self, config: Qwen3Config):
        ...
        self.q_proj = nn.Linear(self.hidden_size, 4096, bias=False)
        self.k_proj = nn.Linear(self.hidden_size, 1024, bias=False)
        self.v_proj = nn.Linear(self.hidden_size, 1024, bias=False)
        self.o_proj = nn.Linear(4096, self.hidden_size, bias=False)

        self.head_dim = 4096 // self.num_heads
        kv_head_dim = 1024 // self.num_key_value_heads
        if kv_head_dim != self.head_dim:
            raise ValueError(
                f"Q/K/V head dims must match, got q_head_dim={self.head_dim}, kv_head_dim={kv_head_dim}"
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        self.qjl.device = hidden_states.device
        self.qjl_initial_layers.device = hidden_states.device
        ...
        if past_key_value is not None and use_cache:
            ...
            value_quant_tokens = past_key_value[5]
            ...
            if kv_quant.key_states_quant is not None:
                desired_value_quant_tokens = kv_quant.key_states_quant.shape[2] * self.group_size
            ...
            if delta_tokens > 0:
                ...
                if value_states_quant is None:
                    ...
                    value_states_quant = torch.cat((value_states_quant, quant_new), dim=2)
            ...
            att_qk = kv_quant.attention_score(query_states.contiguous())
            attn_weights = att_qk.mul_(1.0 / math.sqrt(self.head_dim))
            ...
            attn_output = attn_output_new if attn_output is None else (attn_output + attn_output_new)
            ...
            value_states_full = torch.cat((value_states_full, value_states), dim=2).contiguous()
            ...
            if input_dtype == torch.float32:
                ...
                key_states_expanded = key_states_expanded.to(target_dtype)
                value_states_expanded = value_states_expanded.to(target_dtype)
            ...
            if use_cache:
                ...
                if idx is not None and idx < self.initial_layers_count:
                    kv_quant = QJLKeyQuantizer(
                ...
                if value_states.shape[-2] <= self.buffer_size:
                    ...
                    torch.cuda.set_device(value_states_quant.device)

                    value_states_quant, value_scale, value_mn = triton_quantize_and_pack_along_last_dim(
                        value_states_quant, self.group_size, self.value_quantization_bits
                    )

                if value_states_quant is not None:
                    value_quant_tokens = value_states_quant.shape[2] * self.group_size
                else:
                    value_quant_tokens = 0
...
class Qwen3DecoderLayer_QJL(nn.Module):
    def __init__(self, config: Qwen3Config):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = Qwen3Attention_QJL(config=config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if "padding_mask" in kwargs:
            warnings.warn(
                "Passing `padding_mask` as kwarg is deprecated; "
                "use explicit `padding_mask` argument instead."
            )
...
class Qwen3Model_QJL(Qwen3PreTrainedModel):
    def __init__(self, config: Qwen3Config):
        ...
        self.layers = nn.ModuleList(
            [Qwen3DecoderLayer_QJL(config) for _ in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        has_padding = (
            ...
            and torch.any(attention_mask_2d == 0)
        )
...
class Qwen3ForCausalLM_QJL(GenerationMixin, Qwen3PreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        Qwen3PreTrainedModel.__init__(self, config)
        self.model = Qwen3Model_QJL(config)
        self.vocab_size = config.vocab_size
        ...
        for layer_past in past_key_values:
            *tensor_states, kv_len = layer_past
            new_tensor_states = tuple(
                ...
                if isinstance(past_state, torch.Tensor)
                else past_state
