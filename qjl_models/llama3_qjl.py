...
class LlamaAttention_QJL(nn.Module):
    def __init__(self, config: LlamaConfig):
        ...
        self.qjl_initial_layers = QJLSketch(dim=(128, config.key_quantization_bits_initial_layers), dim_outlier=0,
                                      rot=True)
        self.key_quantization_bits = getattr(config, "key_quantization_bits", 4)
        ...
        self.value_quantization_bits = getattr(config, "value_quantization_bits", 4)
        self.group_size = getattr(config, "group_size", 64)
        self.buffer_size = getattr(config, "buffer_size", 128)
        ...
        if past_key_value is not None and use_cache:
            ...
            if use_flash:
                ...
                attn_output = _flash_attention_forward(
                    ...
                    repeat_kv(key_states, self.num_key_value_groups).transpose(1, 2),
                    repeat_kv(value_states, self.num_key_value_groups).transpose(1, 2),
                    attn_mask_for_flash,
                    q_len,
            ...
            if use_cache:
                ...
                if value_states.shape[-2] <= self.buffer_size:
                    ...
                    value_quant_tokens = value_states_quant.shape[2]
            else:
                kv_quant = None
                value_states_quant = None
            ...
            past_key_value = (
                (
                    ...
                    value_scale,
                    value_mn,
                    value_quant_tokens if use_cache else None,
                    kv_seq_len,
                )
                if use_cache
                else None
...
class LlamaDecoderLayer_QJL(nn.Module):
    def __init__(self, config: LlamaConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = LlamaAttention_QJL(config=config)
...
class LlamaModel_QJL(LlamaPreTrainedModel):
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaDecoderLayer_QJL(config) for _ in range(config.num_hidden_layers)]
        )
...
class LlamaForCausalLM_QJL(GenerationMixin, LlamaPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        LlamaPreTrainedModel.__init__(self, config)
        self.model = LlamaModel_QJL(config)
        self.vocab_size = config.vocab_size
            ...
            *states, kv_len = layer_past
            ...
            for s in states:
                if isinstance(s, torch.Tensor):
                    ...
                    new_states.append(s)
            reordered_past += ((*new_states, kv_len),)
