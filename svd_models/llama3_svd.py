...
class LlamaAttention_SVD(nn.Module):
    ...
    def set_cache_quantization_fns(
        self,
        quantize_fn: Optional[Callable[[torch.Tensor, torch.dtype], torch.Tensor]] = None,
        dequantize_fn: Optional[Callable[[torch.Tensor, torch.dtype], torch.Tensor]] = None,
    ):
        pass
    ...
    def _svd_decompose_kv_chunk(
        ...
        with autocast_ctx:
            if self.svd_method == "exact":
                ...
                raise ValueError(
                    f"Unknown svd_method='{self.svd_method}'. "
                    f"Supported: 'exact', 'random'."
                )
        ...
        **kwargs,
        ...
        if past_key_value is not None and use_cache:
            ...
            if len_svd_tokens > 0 and vU is not None and vVt is not None:
                ...
                ctx_svd = torch.matmul(tmp, vVt)
                ctx_svd = ctx_svd.sum(dim=0)
            ...
            if len_resid_tokens > 0 and v_resid_code is not None:
                ...
                if context is None:
                    ...
                    context = context.add_(ctx_resid)
...
class LlamaDecoderLayer_SVD(nn.Module):
    def __init__(self, config: LlamaConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = LlamaAttention_SVD(config=config)
    ...
    def set_cache_quantization_fns(
        ...
        self.self_attn.set_cache_quantization_fns(quantize_fn, dequantize_fn)
...
class LlamaModel_SVD(LlamaPreTrainedModel):
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaDecoderLayer_SVD(config) for _ in range(config.num_hidden_layers)]
        )
    ...
    def set_cache_quantization_fns(
        ...
        for layer in self.layers:
            layer.set_cache_quantization_fns(quantize_fn, dequantize_fn)
...
class LlamaForCausalLM_SVD(GenerationMixin, LlamaPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config: LlamaConfig):
        GenerationMixin.__init__(self)
        LlamaPreTrainedModel.__init__(self, config)
        self.model = LlamaModel_SVD(config)
        self.vocab_size = config.vocab_size
    ...
    def set_cache_quantization_fns(
        ...
        self.model.set_cache_quantization_fns(quantize_fn, dequantize_fn)
