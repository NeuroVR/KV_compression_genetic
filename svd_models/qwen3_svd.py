...
class Qwen3Attention_SVD(nn.Module):
    def __init__(self, config: Qwen3Config):
        ...
        self.head_dim = 1024 // self.num_key_value_heads
        self.q_head_dim = 4096 // self.num_heads
        if (self.num_heads * self.q_head_dim) != 4096:
            raise ValueError(
                f"4096 должно быть кратно num_heads (получено num_heads={self.num_heads})"
            )
        if self.q_head_dim != self.head_dim:
            raise ValueError(
                f"Ожидается q_head_dim == head_dim, "
                f"но получено q_head_dim={self.q_head_dim}, head_dim={self.head_dim}"
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.pretraining_tp > 1:
            q_out_dim = self.num_heads * self.q_head_dim
            kv_out_dim = self.num_key_value_heads * self.head_dim
            key_value_slicing = kv_out_dim // self.pretraining_tp

            query_slices = self.q_proj.weight.split(
                q_out_dim // self.pretraining_tp, dim=0
            )
        ...
        attn_output = attn_output.reshape(bsz, q_len, self.num_heads * self.q_head_dim)
...
class Qwen3DecoderLayer_SVD(nn.Module):
    def __init__(self, config: Qwen3Config):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = Qwen3Attention_SVD(config=config)
...
class Qwen3Model_SVD(Qwen3PreTrainedModel):
    def __init__(self, config: Qwen3Config):
        ...
        self.layers = nn.ModuleList(
            [Qwen3DecoderLayer_SVD(config) for _ in range(config.num_hidden_layers)]
        )
...
class Qwen3ForCausalLM_SVD(GenerationMixin, Qwen3PreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config: Qwen3Config):
        GenerationMixin.__init__(self)
        Qwen3PreTrainedModel.__init__(self, config)
        self.model = Qwen3Model_SVD(config)
        self.vocab_size = config.vocab_size
