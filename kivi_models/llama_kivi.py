...
class LlamaAttention_KIVI(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.kivi_chunk_size = int(getattr(config, "kivi_chunk_size", 2048))
    ...
    def _quantize_k_cache_chunked(
        self,
        k_data_transposed: torch.Tensor,
    ):
        assert k_data_transposed.dim() == 4
        B, n_kv, Hd, T_total = k_data_transposed.shape

        feat_per_int = 32 // self.k_bits
        base = math.lcm(self.group_size, feat_per_int)

        if T_total <= self.kivi_chunk_size:
            return triton_quantize_and_pack_along_last_dim(
                k_data_transposed, self.group_size, self.k_bits
            )

        chunk_len = min(self.kivi_chunk_size, T_total)
        chunk_len = max(base, (chunk_len // base) * base)

        codes = []
        scales = []
        mns = []

        for start in range(0, T_total, chunk_len):
            end = min(T_total, start + chunk_len)
            chunk = k_data_transposed[..., start:end].contiguous()

            c, s, m = triton_quantize_and_pack_along_last_dim(
                chunk, self.group_size, self.k_bits
            )
            codes.append(c)
            scales.append(s)
            mns.append(m)
        ...
        mn = torch.cat(mns, dim=-1)

        return code, scale, mn

    def _quantize_v_cache_chunked(
        self,
        v_data: torch.Tensor,
    ):
        assert v_data.dim() == 4
        B, n_kv, T_total, D = v_data.shape

        if T_total <= self.kivi_chunk_size:
            return triton_quantize_and_pack_along_last_dim(
                v_data, self.group_size, self.v_bits
            )
        ...
        chunk_len = self.kivi_chunk_size

        for start in range(0, T_total, chunk_len):
            end = min(T_total, start + chunk_len)
            chunk = v_data[:, :, start:end, :].contiguous()
            c, s, m = triton_quantize_and_pack_along_last_dim(
                chunk, self.group_size, self.v_bits
            )
            codes.append(c)
            scales.append(s)
            mns.append(m)

        code = torch.cat(codes, dim=2)   
        scale = torch.cat(scales, dim=2)
        mn = torch.cat(mns, dim=2)
        ...
        if past_key_value is not None and use_cache:
            ...
            if use_cache:               
                ...
                if key_states_quant is not None:
                    k_data = key_states_quant.transpose(2, 3).contiguous()
                    (
                        key_states_quant_trans,
                        key_scale_trans,
                        key_mn_trans,
                    ) = self._quantize_k_cache_chunked(k_data)
                else:
                ...
                if value_states.shape[-2] <= self.residual_length:
                    ...
                    value_states_quant_raw = value_states[
                    ...
                    (
                        ...
                        value_scale,
                        value_mn,
                    ) = self._quantize_v_cache_chunked(value_states_quant_raw)
            else:
                key_states_quant_trans = None
                key_states_full = None
                key_scale_trans = None
...
class LlamaDecoderLayer_KIVI(nn.Module):
    def __init__(self, config: LlamaConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = LlamaFlashAttention_KIVI(config=config)
...
class LlamaModel_KIVI(LlamaPreTrainedModel):
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaDecoderLayer_KIVI(config) for _ in range(config.num_hidden_layers)]
        )
...
class LlamaForCausalLM_KIVI(GenerationMixin, LlamaPreTrainedModel):
