...
class Qwen3Attention_KIVI(nn.Module):
...
class Qwen3FlashAttention_KIVI(Qwen3Attention_KIVI):
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
...
class Qwen3DecoderLayer_KIVI(nn.Module):
    def __init__(self, config: Qwen3Config):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = Qwen3FlashAttention_KIVI(config=config)
...
class Qwen3Model_KIVI(Qwen3PreTrainedModel):
    def __init__(self, config: Qwen3Config):
        ...
        self.layers = nn.ModuleList(
            [Qwen3DecoderLayer_KIVI(config) for _ in range(config.num_hidden_layers)]
        )
...
class Qwen3ForCausalLM_KIVI(GenerationMixin, Qwen3PreTrainedModel):
