...
class LlamaAttention_FAISSKV(nn.Module):
    """
    Вариант self-attention, где K/V могут быть квантованы с помощью Product Quantizer (FAISS PQ).
    Новое:
      - метод build_pq_from_samples(...) на уровне слоя.
    """
    ...
    # --------- НОВОЕ: слой сам умеет строить PQ по собранным данным ---------
    ...
    def build_pq_from_samples(
        ...
        nbits: бит на саб-вектор (определяет ksub = 2**nbits)
        """
...
class LlamaDecoderLayer_FAISSKV(nn.Module):
    def __init__(self, config: LlamaConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = LlamaAttention_FAISSKV(config=config)
...
class LlamaModel_FAISSKV(LlamaPreTrainedModel):
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaDecoderLayer_FAISSKV(config) for _ in range(config.num_hidden_layers)]
        )
...
class LlamaForCausalLM_FAISSKV(GenerationMixin, LlamaPreTrainedModel):
    """
    Полноценная LM‑обёртка вокруг LlamaModel_FAISSKV.
    Теперь обучение PQ делегировано слою через attention.build_pq_from_samples.
    """
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.model = LlamaModel_FAISSKV(config)
        self.vocab_size = config.vocab_size
    ...
    def _setup_faisskv_after_loading(self):
        """
        Остаётся для обратной совместимости (если centroids уже загружены с чекпоинта).
        """
        ...
        for layer in self.model.layers:
            attn: LlamaAttention_FAISSKV = layer.self_attn
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        model: "LlamaForCausalLM_FAISSKV" = super().from_pretrained(
    ...
    def build_faiss_indices(
        ...
        """
        Собирает KV для заданного количества примеров и обучает PQ для верхних num_quantize_layers слоёв.
        Логика обучения вынесена в attention.build_pq_from_samples(...).
        """
        ...
        for layer_idx in range(start_layer, num_layers):
            ...
            attn: LlamaAttention_FAISSKV = self.model.layers[layer_idx].self_attn
