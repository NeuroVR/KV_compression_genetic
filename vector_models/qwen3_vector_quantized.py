...
class Qwen3Attention_FAISSKV(nn.Module):
    def __init__(self, config: Qwen3Config):
        ...
        # ВАЖНО: здесь размеры зашиты под конкретную модель Qwen3.
        # Оставляю как в вашем исходном коде.
        self.q_proj = nn.Linear(2560, 4096, bias=False)
        self.k_proj = nn.Linear(2560, 1024, bias=False)
        self.v_proj = nn.Linear(2560, 1024, bias=False)
        self.o_proj = nn.Linear(4096, 2560, bias=False)

        self.q_head_dim = self.q_proj.out_features // self.num_heads
        self.kv_head_dim = self.k_proj.out_features // self.num_key_value_heads
        if self.q_head_dim != self.kv_head_dim:
            raise ValueError(
                f"q_head_dim={self.q_head_dim} != kv_head_dim={self.kv_head_dim}, "
                f"это ломает flash_attention."
            )
        self.head_dim = self.kv_head_dim

        norm_dim = self.kv_head_dim
        self.q_norm = Qwen3RMSNorm(norm_dim, eps=config.rms_norm_eps)
        self.k_norm = Qwen3RMSNorm(norm_dim, eps=config.rms_norm_eps)
        ...
        self.pq_max_tokens: int = 2000  # ограничение по токенам при кодировании
    ...
    def load_state_dict(
        ...
        # Позволяет загрузить уже обученные центроиды из чекпоинта
    ...
    # ---------- НОВОЕ: послойное обучение PQ, как в Llama ----------
    ...
    def build_pq_from_samples(
        ...
        Аргументы:
          ...
          nbits: бит на саб-вектор (ksub = 2**nbits)
          quantize_k, quantize_v: включить/выключить PQ для K/V на данном слое
          max_train_vectors: максимум векторов для обучения (рандомный сабсемплинг)
        """
        if device is None:
    ...
    def _encode_with_pq(
        ...
        assert M_c == M_local
        assert d == M_local * dsub, f"d={d}, M={M_local}, dsub={dsub}"
        ...
        for start in range(0, seq, chunk_size):
            ...
            dist2 = x_norm2.unsqueeze(-1) + centroids_norm2.unsqueeze(0) - 2.0 * dots
    ...
    def _compute_attn_bias(
        ...
        causal_2d = causal_2d[past_kv_len:, :]
        if total_len != kv_seq_len:
            causal_2d = causal_2d[:, :kv_seq_len]

        return causal_2d.view(1, 1, q_len, kv_seq_len)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if "padding_mask" in kwargs:
            warnings.warn(
                "Passing `padding_mask` is deprecated. "
                "Используйте `attention_mask`."
            )

        bsz, q_len, _ = hidden_states.size()

        # В этой реализации pretraining_tp не используется
        self.pretraining_tp = 1
        if self.pretraining_tp > 1:
            raise NotImplementedError("pretraining_tp > 1 не реализован.")
        else:
        ...
        # --- Разбор past_key_value (учёт PQ-кода) ---
        past_k = past_v = None
        ...
        # --- Сбор KV для обучения PQ (опционально) ---
        if collect_kv_for_faiss and kv_collector is not None and self.layer_idx is not None:
            d = key_states.shape[-1]
            k_flat = key_states.contiguous().transpose(1, 2).reshape(-1, d)
            v_flat = value_states.contiguous().transpose(1, 2).reshape(-1, d)
        ...
        # --- Конкатенация past и текущих K/V ---
        target_dtype = self.q_proj.weight.dtype

        if past_k is not None:
            past_k = torch.cat([past_k, key_states], dim=2).contiguous().to(target_dtype)
            key_states_full = past_k
        else:
            key_states_full = key_states

        if past_v is not None:
            past_v = torch.cat([past_v, value_states], dim=2).contiguous().to(target_dtype)
            value_states_full = past_v
        else:
            value_states_full = value_states

        attn_weights = None

        # --- Внимание: flash или обычное ---
        use_flash = (
            self.use_flash
        ...
        if use_flash:
            attn_mask_for_flash = self._build_flash_attn_mask(attention_mask)
...
class Qwen3DecoderLayer_FAISSKV(nn.Module):
    def __init__(self, config: Qwen3Config):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = Qwen3Attention_FAISSKV(config=config)
...
class Qwen3Model_FAISSKV(Qwen3PreTrainedModel):
    def __init__(self, config: Qwen3Config):
        Qwen3PreTrainedModel.__init__(self, config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size

        # Гарантируем корректную настройку attention implementation
        config._attn_implementation = "sdpa"
        ...
        self.layers = nn.ModuleList(
            [Qwen3DecoderLayer_FAISSKV(config) for _ in range(config.num_hidden_layers)]
        )
...
class Qwen3ForCausalLM_FAISSKV(GenerationMixin, Qwen3PreTrainedModel):
    ...
    def __init__(self, config: Qwen3Config):
        ...
        self.model = Qwen3Model_FAISSKV(config)
        self.vocab_size = config.vocab_size
    ...
    def _setup_faisskv_after_loading(self):
        """
        Инициализация параметров PQ из сохранённых буферов (обратная совместимость).
        """
        ...
        for layer in self.model.layers:
            attn: Qwen3Attention_FAISSKV = layer.self_attn
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        model: "Qwen3ForCausalLM_FAISSKV" = super().from_pretrained(
    ...
    # --------- ПОСТРОЕНИЕ FAISS‑PQ (аналогично Llama) ---------
    ...
    def build_faiss_indices(
        ...
        """
        Собирает KV для заданного количества примеров и обучает PQ
        для верхних num_quantize_layers слоёв, аналогично реализации для Llama.

        ВАЖНО: сама логика обучения перенесена в
        Qwen3Attention_FAISSKV.build_pq_from_samples(...)
        """
        ...
        example_attn: Qwen3Attention_FAISSKV = self.model.layers[0].self_attn
        head_dim = example_attn.kv_head_dim
        num_layers = self.config.num_hidden_layers

        if m is None:
            m = head_dim  # по умолчанию M = head_dim => dsub=1
        ...
        for layer_idx in range(start_layer, num_layers):
            ...
            layer_attn: Qwen3Attention_FAISSKV = self.model.layers[layer_idx].self_attn

            if len(k_list) == 0:
                print(f"[FAISS] Layer {layer_idx}: нет собранных KV, пропускаем.")
                layer_attn.quantize_k = False
                layer_attn.quantize_v = False
                continue
            ...
            layer_attn.build_pq_from_samples(
                k_data=k_data,
                v_data=v_data,
                m=m,
                nbits=nbits,
        ...
        # Сохраняем общие параметры в конфиг (для последующей загрузки)
