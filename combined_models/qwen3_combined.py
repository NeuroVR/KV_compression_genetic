...
from kivi_models.qwen3_kivi import Qwen3FlashAttention_KIVI
from qjl_models.qwen3_qjl import Qwen3Attention_QJL
from svd_models.qwen3_svd import Qwen3Attention_SVD
from vector_models.qwen3_vector_quantized import Qwen3Attention_FAISSKV
...
class Qwen3Attention(nn.Module):
    def __init__(self, config: Qwen3Config):
        ...
        if kv_head_dim != self.head_dim:
            raise ValueError(
                f"Q/K/V head dims must match, got q_head_dim={self.head_dim}, "
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.pretraining_tp > 1:
            ...
            q = [F.linear(hidden_states, qs) for qs in query_slices]
            q = torch.cat(q, dim=-1)

            k = [F.linear(hidden_states, ks) for ks in key_slices]
            k = torch.cat(k, dim=-1)

            v = [F.linear(hidden_states, vs) for vs in value_slices]
            v = torch.cat(v, dim=-1)
        else:
...
class Qwen3DecoderLayer_MIXEDKV(nn.Module):
    def __init__(
        self,
        ...
        if self.kv_type == "kivi":
            self.self_attn = Qwen3FlashAttention_KIVI(config=layer_config)

        elif self.kv_type == "qjl":
            self.self_attn = Qwen3Attention_QJL(config=layer_config)

        elif self.kv_type == "svd":
            self.self_attn = Qwen3Attention_SVD(config=layer_config)

        elif self.kv_type == "faiss":
            self.self_attn = Qwen3Attention_FAISSKV(config=layer_config)
            if hasattr(self.self_attn, "layer_idx"):
                self.self_attn.layer_idx = layer_idx

        elif self.kv_type == "none":
            self.self_attn = Qwen3Attention(config=layer_config)
        ...
        self.mlp = Qwen3MLP(layer_config)
...
class Qwen3Model_MIXEDKV(Qwen3PreTrainedModel):
    def __init__(self, config: Qwen3Config):
...
class Qwen3ForCausalLM_MIXEDKV(GenerationMixin, Qwen3PreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config: Qwen3Config):
        Qwen3PreTrainedModel.__init__(self, config)
        self.model = Qwen3Model_MIXEDKV(config)
        self.vocab_size = config.vocab_size
    ...
    def build_faiss_indices(
        ...
        if head_dim % m != 0:
            raise ValueError(
                f"[MIXEDKV][FAISS] head_dim={head_dim} must be divisible by m={m}."
            )
        ...
        faiss_layer_indices = [
            ...
            if isinstance(layer.self_attn, Qwen3Attention_FAISSKV)
        ]
