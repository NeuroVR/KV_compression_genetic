...
from kivi_models.qwen2_kivi import Qwen2FlashAttention_KIVI
from qjl_models.qwen2_qjl import Qwen2Attention_QJL
from svd_models.qwen2_svd import Qwen2Attention_SVD
from vector_models.qwen2_vector_quantized import Qwen2Attention_FAISSKV
        ...
        **kwargs,
        ...
        if past_key_value is not None and use_cache:
            if len(past_key_value) != 3:
                raise ValueError(
                    f"Ожидаю past_key_value формата (k, v, kv_len), "
                    f"получено len={len(past_key_value)}"
                )
        ...
        if past_key_value is not None and use_cache:
            ...
            if use_cache:
                present_key_value = (key_states_full, value_states_full, kv_seq_len)
            else:
            ...
            if use_cache:
                present_key_value = (key_states, value_states, kv_seq_len)
            else:
                present_key_value = None
...
class Qwen2DecoderLayer_MIXEDKV(nn.Module):
    def __init__(
        self,
        ...
        if self.kv_type == "kivi":
            self.self_attn = Qwen2FlashAttention_KIVI(config=layer_config)

        elif self.kv_type == "qjl":
            self.self_attn = Qwen2Attention_QJL(config=layer_config)

        elif self.kv_type == "svd":
            self.self_attn = Qwen2Attention_SVD(config=layer_config)

        elif self.kv_type == "faiss":
            self.self_attn = Qwen2Attention_FAISSKV(config=layer_config)
            if hasattr(self.self_attn, "layer_idx"):
                self.self_attn.layer_idx = layer_idx

        elif self.kv_type == "none":
            self.self_attn = Qwen2Attention(config=layer_config)
        else:
            raise ValueError(
        ...
        self.mlp = Qwen2MLP(layer_config)
...
class Qwen2Model_MIXEDKV(Qwen2PreTrainedModel):
    def __init__(self, config: Qwen2Config):
...
class Qwen2ForCausalLM_MIXEDKV(GenerationMixin, Qwen2PreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config: Qwen2Config):
        Qwen2PreTrainedModel.__init__(self, config)
        self.model = Qwen2Model_MIXEDKV(config)
        self.vocab_size = config.vocab_size
    ...
    def build_faiss_indices(
        ...
        faiss_layer_indices = [
            ...
            if isinstance(layer.self_attn, Qwen2Attention_FAISSKV)
        ]
        if not faiss_layer_indices:
            print("[MIXEDKV][FAISS] Нет слоёв с kv_type='faiss'; ничего обучать не будем.")
            return
        ...
        if not target_layers:
            print(
                "[MIXEDKV][FAISS] В верхних слоях нет faiss-слоёв; "
                "ни один слой не будет квантован."
            )
            return
        ...
        for layer_idx in target_layers:
            ...
            if len(k_list) == 0:
                print(f"[MIXEDKV][FAISS] Layer {layer_idx}: нет собранных KV, пропускаем.")
        ...
        print("[MIXEDKV][FAISS] Обучение ProductQuantizer завершено.")
